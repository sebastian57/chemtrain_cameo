...
import inspect
import os
from typing import Optional, Callable, Tuple, TYPE_CHECKING, Any
...
_NEIGHBOR_DEBUG_ENABLED = str(
    os.environ.get("CHEMTRAIN_DEBUG_NEIGHBOR", "1")
).strip().lower() in ("1", "true", "yes", "on")
_NEIGHBOR_DEBUG_RANK0_ONLY = str(
    os.environ.get("CHEMTRAIN_DEBUG_NEIGHBOR_RANK0_ONLY", "1")
).strip().lower() in ("1", "true", "yes", "on")
_SPARSE_GRAPH_INPUT_BUDGET = [1]
_SPARSE_GRAPH_OUTPUT_BUDGET = [1]


def _sparse_graph_debug_rank_ok() -> bool:
    if not _NEIGHBOR_DEBUG_ENABLED:
        return False
    if not _NEIGHBOR_DEBUG_RANK0_ONLY:
        return True
    return jax.process_index() == 0


def _sparse_graph_output_debug_cb(
    n_particles: Any,
    max_neighbors: Any,
    max_edges_cfg: Any,
    e_capacity: Any,
    ...
    if _SPARSE_GRAPH_OUTPUT_BUDGET[0] <= 0 or not _sparse_graph_debug_rank_ok():
        return
    _SPARSE_GRAPH_OUTPUT_BUDGET[0] -= 1
    print(
        "[NeighborDebug][dense_to_sparse][exit] "
        f"N={int(n_particles)} M_slots={int(max_neighbors)} "
        f"max_edges={int(max_edges_cfg)} E_capacity={int(e_capacity)} "
        f"n_edges={int(n_edges_true)} capped={bool(capped)}"
    )
...
def sparse_graph_from_neighborlist(displacement_fn: Callable,
    ...
    if _SPARSE_GRAPH_INPUT_BUDGET[0] > 0 and _sparse_graph_debug_rank_ok():
        _SPARSE_GRAPH_INPUT_BUDGET[0] -= 1
        print(
            "[NeighborDebug][dense_to_sparse][entry] "
            f"neighbor.idx.shape=({int(n_particles)}, {int(max_neighbors)}) "
            f"max_edges={max_edges} r_cutoff={r_cutoff}"
        )
    ...
    if _NEIGHBOR_DEBUG_ENABLED:
        jax.debug.callback(
            _sparse_graph_output_debug_cb,
            jnp.asarray(n_particles, dtype=jnp.int32),
            jnp.asarray(max_neighbors, dtype=jnp.int32),
            jnp.asarray(max_edges, dtype=jnp.int32),
            jnp.asarray(idx_i.shape[0], dtype=jnp.int32),
            jnp.asarray(n_edges, dtype=jnp.int32),
            jnp.asarray(too_many_edges_error_code, dtype=jnp.bool_),
        )
...
def subtract_topology_from_neighbor_list(nbrs: partition.NeighborList,
                                         topology: "Topology",
                                         bond_mask = None,
                                         angle_mask = None,
                                         dihedral_mask = None):
    ...
    if bond_mask is not None:
        ...
        bond_mask = bond_mask_2

    angle_idx, _, angle_mask_2 = topology.get_angles()
    if angle_mask is not None:
        angle_mask = jnp.logical_and(angle_mask, angle_mask_2)
    else:
        angle_mask = angle_mask_2

    dihedral_idx, _, dihedral_mask_2 = topology.get_dihedrals()
    # The dihedral mask might be vectorized. It sufficies, if a single
    # dihedral term for that dihedral is nonzero
    dihedral_mask_2 = jnp.any(dihedral_mask_2, axis=-1)
    if dihedral_mask is not None:
        dihedral_mask = jnp.logical_and(dihedral_mask, dihedral_mask_2)
    else:
