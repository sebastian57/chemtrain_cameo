...
def mask_dense(idx, mask=None, segment_id=None):
    # Mask out edges to self
    self_mask = (idx == jnp.arange(idx.shape[0])[:, jnp.newaxis])
    n_atoms = idx.shape[0]
    valid_idx = (idx >= 0) & (idx < n_atoms)
    idx_safe = jnp.where(valid_idx, idx, 0)

    total_mask = jnp.logical_or(self_mask, jnp.logical_not(valid_idx))

    if mask is not None:
        mask = jnp.asarray(mask, dtype=jnp.bool_)
        sender_valid = mask[:, jnp.newaxis]
        receiver_valid = mask[idx_safe]
        total_mask = jnp.logical_or(total_mask, jnp.logical_not(sender_valid))
        total_mask = jnp.logical_or(total_mask, jnp.logical_not(receiver_valid))

    if segment_id is not None:
        segment_id = jnp.asarray(segment_id, dtype=jnp.int32)
        sender_seg = segment_id[:, jnp.newaxis]
        receiver_seg = segment_id[idx_safe]
        same_segment = sender_seg == receiver_seg
        valid_segment = (sender_seg >= 0) & (receiver_seg >= 0)
        total_mask = jnp.logical_or(
            total_mask, jnp.logical_not(same_segment & valid_segment)
        )

    return jnp.where(total_mask, n_atoms, idx)
...
def mask_neighbor_list(nbrs: partition.NeighborList,
                       mask: Array = None,
                       segment_id: Array = None) -> partition.NeighborList:
    ...
    Args:
        ...
        segment_id: Optional integer segment ID per particle. If provided,
            only edges with matching non-negative sender/receiver segment IDs
            are kept.
    ...
    if mask is None:
        mask = jnp.ones((n_atoms,), dtype=jnp.bool_)
    else:
        mask = jnp.asarray(mask, dtype=jnp.bool_)
    ...
    def mask_sparse(idx, mask, segment_id):
        ...
        senders_safe = jnp.clip(senders, 0, n_atoms - 1)
        receivers_safe = jnp.clip(receivers, 0, n_atoms - 1)
        valid_edge = jnp.logical_and(valid_edge, mask[senders_safe])
        valid_edge = jnp.logical_and(valid_edge, mask[receivers_safe])

        if segment_id is not None:
            sender_seg = segment_id[senders_safe]
            receiver_seg = segment_id[receivers_safe]
            valid_segment = (sender_seg >= 0) & (receiver_seg >= 0)
            same_segment = sender_seg == receiver_seg
            valid_edge = jnp.logical_and(valid_edge, valid_segment & same_segment)

        return jnp.where(valid_edge[jnp.newaxis, :], idx, n_atoms)

    if nbrs.format == partition.NeighborListFormat.Dense:
        new_idx = mask_dense(nbrs.idx, mask=mask, segment_id=segment_id)
    else:
        new_idx = mask_sparse(nbrs.idx, mask, segment_id)
