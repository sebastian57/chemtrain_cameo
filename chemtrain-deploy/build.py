...
def main():
  ...
  add_boolean_argument(
      ...
      default=False,
      help_str=(
          "Load the GPU PJRT plugin from the jax wheels. Deprecated for the "
          "modernized stack; source-built plugins are the supported path."
      ),
  )
  add_boolean_argument(
      parser,
  ...
  apply_build_profile_defaults(args)
  logger.info("Using build profile: %s", args.build_profile)

  if os.environ.get("HERMETIC_PYTHON_VERSION") is None:
    python_version = f"{sys.version_info[0]}.{sys.version_info[1]}"
    os.environ["HERMETIC_PYTHON_VERSION"] = python_version
    logger.info("Defaulting HERMETIC_PYTHON_VERSION to %s", python_version)
  ...
  if args.enable_cuda:
      ...
      if args.build_cuda_with_clang:
        ...
        if not args.use_clang:
          build_options.append("--action_env=TF_NVCC_CLANG=0")
  ...
  if args.build_gpu_pjrt_plugin:
      ...
      cuda_major = args.cuda_version.split('.')[0]
      (out_dir / f"pjrt_plugin.xla_cuda{cuda_major}.so").write_bytes(
      ...
      logger.warning(
          "Loading the PJRT plugin from the installed JAX wheel is deprecated "
          "for the modernized stack. Prefer --build_gpu_pjrt_plugin so the "
          "connector and PJRT plugin are built from the same checkout."
      )
      out_dir = pathlib.Path("./lib")
