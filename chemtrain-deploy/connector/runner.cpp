...
namespace jcn {
    ...
    void Runner::initialize() {
        ...
        auto load_plugin = [&](const std::string& path) -> bool {
            std::smatch match;
            if (!std::regex_search(path, match, pattern)) {
                std::cerr << "Failed to infer PJRT backend from plugin path: " << path << std::endl;
                return false;
            }

            absl::StatusOr<const PJRT_Api*> status_or_api =
                pjrt::LoadPjrtPlugin(match.str(1), path);
            if (!status_or_api.ok()) {
                std::cerr << "Failed to load PJRT plugin: "
                          << status_or_api.status().ToString() << std::endl;
                return false;
            }

            logger.log(LogLevel::INFO, "Loaded PJRT plugin " + match.str(1));
            return true;
        };

        if (const char* plugin_file = std::getenv("JCN_PJRT_PLUGIN");
            plugin_file != nullptr && plugin_file[0] != '\0') {
            if (load_plugin(plugin_file)) {
                return;
            }
        }

        const char* plugin_dir = std::getenv("JCN_PJRT_PATH");
        if (plugin_dir == nullptr || plugin_dir[0] == '\0') {
            std::cerr << "Set JCN_PJRT_PLUGIN or JCN_PJRT_PATH to discover PJRT plugins"
                      << std::endl;
            return;
        }
        ...
        if (dir == nullptr) {
            std::cerr << "Failed to open PJRT plugin directory: " << plugin_dir
                      << std::endl;
            return;
        }
        ...
        while (dirent* entry = readdir(dir)) {
            ...
            std::string path = std::string(plugin_dir) + "/" + filename;
            if (load_plugin(path)) {
                loaded = true;
                break;
            }
        ...
        if (!loaded) {
            std::cerr << "No loadable PJRT plugin found in " << plugin_dir << std::endl;
        }
