...
class MLETrainerTemplate(TrainerInterface):
    ...
    def __init__(self,
        ...
        self._disable_gradient_norm = str(
            os.environ.get("CHEMTRAIN_DISABLE_GRAD_NORM", "0")
        ).lower() in ("1", "true", "yes", "on")
        self._disable_train_target_loss_sync = self._env_flag(
            "CHEMTRAIN_DISABLE_TRAIN_TARGET_LOSS_SYNC", default=False
        )
        self._profile_rank = jax.process_index()
        self._profile_rank0_only = self._env_flag(
            "CHEMTRAIN_PROFILE_RANK0_ONLY", default=True
        )
        self._profile_batch_breakdown = self._env_flag(
            "CHEMTRAIN_PROFILE_BATCH_BREAKDOWN", default=False
        )
        self._profile_update_breakdown = self._env_flag(
            "CHEMTRAIN_PROFILE_UPDATE_BREAKDOWN", default=False
        )
        self._profile_task_timing = self._env_flag(
            "CHEMTRAIN_PROFILE_TASK_TIMING", default=False
        )
        self._profile_batch_limit = int(
            os.environ.get("CHEMTRAIN_PROFILE_BATCH_BREAKDOWN_LIMIT", "0")
        )
        self._profile_active_batch_idx = None
        self._profile_active_batch_enabled = False
    ...
    def _env_flag(name, default=False):
        ...
        return str(os.environ.get(name, default_val)).lower() in (
            "1", "true", "yes", "on"
        )

    @staticmethod
    def _task_name(fn_or_method):
        if inspect.ismethod(fn_or_method):
            cls_name = fn_or_method.__self__.__class__.__name__
            return f"{cls_name}.{fn_or_method.__name__}"
        return getattr(fn_or_method, "__name__", fn_or_method.__class__.__name__)

    def _profile_this_rank(self):
        if not self._profile_rank0_only:
            return True
        return self._profile_rank == 0

    def _profile_this_batch(self, batch_idx):
        if not self._profile_this_rank():
            return False
        return self._profile_batch_limit <= 0 or batch_idx < self._profile_batch_limit
    ...
    def _execute_tasks(self, trigger, *args, **kwargs):
        ...
        profile_task_timing = (
            self._profile_task_timing
            and self._profile_this_rank()
            and (
                self._profile_active_batch_idx is None
                or self._profile_active_batch_enabled
            )
        )
        if profile_task_timing:
        ...
        for fn_or_method in self._tasks[trigger]:
            ...
            if profile_task_timing:
                ...
                print(
                    "[TaskTiming] "
                    f"epoch={self._epoch} "
                    f"batch={self._profile_active_batch_idx} "
                    f"trigger={trigger} "
                    f"fn={self._task_name(fn_or_method)} "
                    f"ms={(t_fn_end - t_fn_start) * 1e3:.3f}"
                )
        ...
        if profile_task_timing:
            ...
            print(
                ...
                f"batch={self._profile_active_batch_idx} "
                f"trigger={trigger} "
                f"total_ms={(t_trigger_end - t_trigger_start) * 1e3:.3f}"
            )
    ...
    def train(self, max_epochs, thresh=None, checkpoint_freq=None):
        ...
        profile_next = str(
            os.environ.get("CHEMTRAIN_PROFILE_DATALOADER_NEXT", "0")
        ).lower() in ("1", "true", "yes", "on")
        profile_limit = int(
            os.environ.get("CHEMTRAIN_PROFILE_DATALOADER_NEXT_LIMIT", "0")
        )
        ...
        with CaptureStdout(self.log_file):
            ...
            for _ in range(start_epoch, end_epoch):
                try:
                    ...
                    batch_iter = iter(self._get_batch())
                    batch_idx = 0
                    while True:
                        profile_batch = self._profile_this_batch(batch_idx)
                        timed_fetch = (
                            (profile_next
                             and self._profile_this_rank()
                             and (profile_limit <= 0 or batch_idx < profile_limit))
                            or (self._profile_batch_breakdown and profile_batch)
                        )
                        if timed_fetch:
                            t_fetch_start = time.perf_counter()
                            if profile_next and self._profile_this_rank():
                                print(
                                    "[DataLoaderTiming] before_next "
                                    f"epoch={self._epoch} batch={batch_idx} "
                                    f"t={t_fetch_start:.6f}"
                                )
                        try:
                        ...
                        if timed_fetch:
                            ...
                            fetch_ms = (t_fetch_end - t_fetch_start) * 1e3
                        if timed_fetch:
                            if profile_next and self._profile_this_rank():
                                print(
                                    "[DataLoaderTiming] after_next  "
                                    f"epoch={self._epoch} batch={batch_idx} "
                                    f"t={t_fetch_end:.6f} "
                        ...
                        self._profile_active_batch_idx = batch_idx
                        self._profile_active_batch_enabled = profile_batch

                        if self._profile_batch_breakdown and profile_batch:
                        ...
                        if self._profile_batch_breakdown and profile_batch:
                            print(
                                "[BatchBreakdown] "
                                f"epoch={self._epoch} batch={batch_idx - 1} "
                                f"fetch_ms={fetch_ms:.3f} "
                                f"pre_batch_ms={(t_pre_end - t_pre_start) * 1e3:.3f} "
                                f"update_ms={(t_update_end - t_update_start) * 1e3:.3f} "
                                f"post_batch_ms={(t_post_end - t_post_start) * 1e3:.3f} "
                                f"loop_ms={(t_post_end - t_batch_start) * 1e3:.3f}"
                            )
                        self._profile_active_batch_idx = None
                        self._profile_active_batch_enabled = False
...
class DataParallelTrainer(MLETrainerTemplate):
    ...
    def __init__(self, loss_fn, model, init_params, optimizer, checkpoint_path,
        ...
        self._grad_accum_steps = max(
            1, int(os.environ.get("CHEMTRAIN_GRAD_ACCUM_STEPS", "1"))
        )
        self._grad_accum_mode = str(
            os.environ.get("CHEMTRAIN_GRAD_ACCUM_MODE", "stack_scan")
        ).strip().lower()
        if self._grad_accum_mode not in ("concat_slice", "stack_scan"):
            raise ValueError(
                "Unsupported CHEMTRAIN_GRAD_ACCUM_MODE="
                f"'{self._grad_accum_mode}'. Expected one of: "
                "concat_slice, stack_scan."
            )
        self._printed_grad_accum_batch_shape = False
        self._parallel_device_count = (
            jax.local_device_count() if disable_shmap else device_count()
        )
        self.batched_model = model
        if disable_shmap:
            self._update_fn = max_likelihood.pmap_update_fn(
                self.batched_model, loss_fn, optimizer, penalty_fn)
            self._evaluate_fn = max_likelihood.pmap_loss_fn(
        ...
        self.batch_size = batch_per_device * self._parallel_device_count
        ...
        self._raw_batches_per_epoch: Dict[str, int] = {}
        self._stage_batch_sizes: Dict[str, int] = {}
        self._get_batch_fns: Dict[str, Callable] = {}

        if self._grad_accum_steps > 1 and self._disable_shmap:
            raise NotImplementedError(
                "CHEMTRAIN_GRAD_ACCUM_STEPS > 1 requires shmap "
                "(disable_shmap=False)."
            )
        if self._grad_accum_steps > 1:
            print(
                "[Training] Gradient accumulation enabled: "
                f"{self._grad_accum_steps} batches/update "
                f"(mode={self._grad_accum_mode})"
            )
    ...
    def set_loader(self,
        ...
        batch_size -= onp.mod(batch_size, self._parallel_device_count)
        ...
        raw_batches = observation_count // batch_size
        self._raw_batches_per_epoch[stage] = raw_batches
        self._stage_batch_sizes[stage] = batch_size

        if stage == "training" and self._grad_accum_steps > 1:
            self._batches_per_epoch[stage] = (
                raw_batches + self._grad_accum_steps - 1
            ) // self._grad_accum_steps
            if raw_batches % self._grad_accum_steps != 0:
                warnings.warn(
                    "Number of training batches per epoch is not divisible by "
                    "CHEMTRAIN_GRAD_ACCUM_STEPS. The last update in each epoch "
                    "will use fewer microbatches and may trigger one extra "
                    "JAX compile for that shape."
                )
        else:
            self._batches_per_epoch[stage] = raw_batches
        self.release_fns[stage] = release

    def _get_batch_stage(self, stage, information=False):
        if not (stage == "training" and self._grad_accum_steps > 1 and not information):
        ...
        def _merge_pending_batches(pending_batches):
            if self._grad_accum_mode == "stack_scan":
                if len(pending_batches) == 1:
                    return pending_batches[0]
                return tree_util.tree_map(
                    lambda *xs: jnp.stack(xs, axis=0), *pending_batches
                )
            return tree_util.tree_map(
                lambda *xs: jnp.concatenate(xs, axis=0), *pending_batches
            )

        pending_batches = []
        raw_batches = self._raw_batches_per_epoch[stage]
        max_raw_batches = min(
            raw_batches, self._batches_per_epoch[stage] * self._grad_accum_steps
        )
        for _ in range(max_raw_batches):
            self._batch_states[stage], train_batch = self._get_batch_fns[stage](
                self._batch_states[stage], information=information)
            pending_batches.append(train_batch)

            if len(pending_batches) == self._grad_accum_steps:
                yield _merge_pending_batches(pending_batches)
                pending_batches = []
    ...
    def _update(self, batch):
        ...
        """
        profile_update = (
            self._profile_update_breakdown
            and self._profile_this_rank()
            and self._profile_active_batch_enabled
        )
        if profile_update:
            t_total_start = time.perf_counter()
            t_part_start = t_total_start

        local_batch_size = self._stage_batch_sizes.get("training", self.batch_size)
        batch_leaf_shape = tree_util.tree_leaves(batch)[0].shape
        if self._grad_accum_mode == "stack_scan" and self._grad_accum_steps > 1:
            if (
                len(batch_leaf_shape) >= 2
                and batch_leaf_shape[0] <= self._grad_accum_steps
                and batch_leaf_shape[1] == local_batch_size
            ):
                microbatch_count = int(batch_leaf_shape[0])
            elif batch_leaf_shape[0] == local_batch_size:
                microbatch_count = 1
            else:
                raise ValueError(
                    "Unexpected stacked microbatch shape for stack_scan mode. "
                    f"Got leading dims={batch_leaf_shape[:2]}, "
                    f"local_batch_size={local_batch_size}, "
                    f"CHEMTRAIN_GRAD_ACCUM_STEPS={self._grad_accum_steps}"
                )
        else:
            current_batch_size = batch_leaf_shape[0]
            microbatch_count = max(1, current_batch_size // local_batch_size)

        if (
            self._grad_accum_steps > 1
            and not self._printed_grad_accum_batch_shape
        ):
            self._printed_grad_accum_batch_shape = True
            print(
                "[Training] Accumulation batch shape check: "
                f"mode={self._grad_accum_mode} "
                f"batch_leaf_shape={batch_leaf_shape} "
                f"local_batch_size={local_batch_size} "
        ...
        params, opt_state, train_loss, curr_grad, per_target_losses = self._update_fn(
            ...
            accum_mode=self._grad_accum_mode,
        )
        if profile_update:
            t_after_update_fn = time.perf_counter()
            update_fn_ms = (t_after_update_fn - t_part_start) * 1e3
            t_part_start = t_after_update_fn
        ...
        for key, val in per_target_losses.items():
            ...
            if self._disable_train_target_loss_sync:
                # Keep list lengths stable without forcing per-batch host sync.
                self.train_target_losses[key].append(onp.nan)
            else:
                self.train_target_losses[key].append(onp.asarray(val))
        if profile_update:
            t_after_targets = time.perf_counter()
            per_target_to_host_ms = (t_after_targets - t_part_start) * 1e3
            t_part_start = t_after_targets

        self.state = self.state.replace(params=params, opt_state=opt_state)
        if profile_update:
            t_after_state = time.perf_counter()
            state_replace_ms = (t_after_state - t_part_start) * 1e3
            t_part_start = t_after_state

        self.train_batch_losses.append(onp.asarray(train_loss))
        if profile_update:
            t_after_train_loss = time.perf_counter()
            train_loss_to_host_ms = (t_after_train_loss - t_part_start) * 1e3
            t_part_start = t_after_train_loss

        if self._disable_gradient_norm:
            self.gradient_norm_history.append(onp.nan)
        else:
            self.gradient_norm_history.append(util.tree_norm(curr_grad))
        if profile_update:
            t_total_end = time.perf_counter()
            grad_norm_ms = (t_total_end - t_part_start) * 1e3
            print(
                "[UpdateBreakdown] "
                f"epoch={self._epoch} "
                f"batch={self._profile_active_batch_idx} "
                f"update_fn_ms={update_fn_ms:.3f} "
                f"per_target_to_host_ms={per_target_to_host_ms:.3f} "
                f"state_replace_ms={state_replace_ms:.3f} "
                f"train_loss_to_host_ms={train_loss_to_host_ms:.3f} "
                f"grad_norm_ms={grad_norm_ms:.3f} "
                f"targets={len(per_target_losses)} "
                f"total_update_ms={(t_total_end - t_total_start) * 1e3:.3f}"
            )
